...
Since Rx 2.0, `Create` accepts `async` functions. If the creation of the observable is blocking because of a `Task`, we can take advantage of C#'s built-in support for asynchronous execution of tasks. Instead of having the subscriber explicitly provide the scheduler that will invoke the subscription, we use the scheduling that is implicit when using `async`/`await`.
...
Finished on threadId:6
```

We see two things in this example. First, we see that subscribing is no longer blocking. The second thing, which is special about using `async`/`await`, is that the delegate was invoked synchronously but the executing thread changed after the first `await`. That's because `async` functions aren't a single function like you might expect. Instead, they are converted into a chain of tasks in [continuation-passing style](http://en.wikipedia.org/wiki/Continuation-passing_style) and the environment assumes the responsibility of scheduling them.
...
* `Scheduler.Default` will schedule the actions to take place on the ThreadPool, TaskPool or a new thread. Which is used depends on which is available on your system.

If you are using WPF, Silverlight or a similar platform with XAML, you can get the Rx-XAML package. Then you will also have access to `DispatcherScheduler.Current` (for some platforms, like Windows Phone, it is `CoreDispatcherScheduler.Current`). This allows you to schedule tasks onto the `Dispatcher` with the common interface, either now or in the future. There is the `SubscribeOnDispatcher()` and `ObserveOnDispatcher()` extension methods to `IObservable<T>`, that also help you access the `Dispatcher`. While they appear useful, you will want to avoid these two methods for production code, and we explain why in the [Testing Rx chapter](/Part 4 - Concurrency/2. Testing Rx.md).
...
Hopefully, we won't ever write such code though, and if we did, our tests would give us quick feedback that things went wrong. More realistically, race conditions often slip into the system at integration points.
...
public partial class Window1 : INotifyPropertyChanged
{
    ...
    {
        Value2 = await _service.GetTemperature().FirstAsync();
    }
}
```

There is only one small problem here in that we block on the Dispatcher thread (`await` is a blocking call), however this manifests itself into a deadlock if the service code is written incorrectly.
...
This odd implementation, with explicit scheduling, will cause the three `OnNext` calls to be scheduled once the `FirstAsync()` observable has terminated; however, that is waiting for an `OnNext` to be called: we are deadlocked.
...
2. Avoid using blocking calls, i.e. `await`
...
Earlier in the book, we looked at how we can use Rx with asynchronous operations. In our example, we just read the entire file into memory. We also referenced Jeffery van Gogh's [blog post](http://blogs.msdn.com/b/jeffva/archive/2010/07/23/rx-on-the-server-part-1-of-n-asynchronous-system-io-stream-reading.aspx), which sadly is now out of date; however, his concepts are still sound. Instead of the `Iterator` method from Jeffery's post, we can use schedulers to achieve the same result.
...
First, let us refresh our memory with how to get from the `FileStream`'s asynchronous methods into Rx.
...
The example above uses the `Task.ToObservable` extension method to create a factory. The factory will take a byte array (`buffer`), an offset (`0`) and a length (`source.Length`); it effectively returns the count of the bytes read as a single-value sequence. When the sequence (reader) is subscribed to, `ReadAsync` will read values, starting from the offset, into the buffer. In this case, we will read the whole file. Once the file has been read into the buffer, the sequence (reader) will push the single value (`bytesRead`) in to the sequence.
...
I would like to highlight a hazard of using the `DispatcherScheduler`. You can construct your own instance of a `DispatcherScheduler` by passing in a reference to a `Dispatcher`. The alternative way is to use the static property `DispatcherScheduler.Current`. This can introduce hard to understand problems if it is not used properly. The static property does not return a reference to a static field, but creates a new instance each time, with the static property `Dispatcher.CurrentDispatcher` as the constructor argument. If you access `Dispatcher.CurrentDispatcher` from a thread that is not the UI thread, it will thus give you a new instance of a Dispatcher, but it will not be the instance you were hoping for.
...
This code may intuitively seem correct, but actually takes away power from consumers of the sequence. When we subscribe to the sequence, we decide that reading a file on the UI thread is a bad idea. So we add in a `SubscribeOn(Scheduler.Default)` to the chain as below:
...
If you do not wish to manage the resources of a thread or an`EventLoopScheduler`, then you can use `NewThreadScheduler`. You can create your own instance of `NewThreadScheduler`. Like `EventLoopScheduler`, you can use the parameterless constructor or provide your own thread factory function. If you do provide your own factory, be careful to set the `IsBackground` property appropriately.
