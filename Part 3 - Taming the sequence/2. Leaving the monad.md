...
You can use the `await` keyword on any `IObservable<T>`, just like you would on a `Task` or `async` method. This causes the execution to block until the observable completes. The value returned by the `await` statement is the last value emitted by the observable.
