...
You can use the `await` keyword on any observable, just like you would on a `Task`. This causes the execution to block until the observable completes and return the last value emitted by the observable. This makes using the `ToTask` unnecessary, unless you need to explicitly aquire references of type `Task<T>`.
...
In the case of an empty sequence, awaiting on it will result in an `InvalidOperationException` being thrown. If the observable fails, the exception that caused the observable to fail will be thrown.
