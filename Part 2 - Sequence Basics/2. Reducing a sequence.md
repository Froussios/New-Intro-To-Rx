...
`TakeUntil` and `SkipUntil` each also have an overload that takes a `DateTimeOffset` parameter. In this case, the sequence will be cut at an absolute moment in time.
