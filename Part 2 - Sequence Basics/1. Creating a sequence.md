...
There is also an [overload that converts a Task (non generic) to an IObservable<Unit>](https://msdn.microsoft.com/en-us/library/system.reactive.threading.tasks.taskobservableextensions). There you will also see some methods for creating Tasks out of IObservables. Suck a task carries the last value of the observable before being completed. Often you won't need to convert to Task as the *await* keyword works directly on IObservable as well.
...
//var task = Task.Factory.StartNew(work); // Without Rx
var observable = Observable.Start(work);
