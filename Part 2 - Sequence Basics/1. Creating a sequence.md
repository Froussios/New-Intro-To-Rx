...
Another convenient method for converting tasks into observables is `Observable.FromAsync`. It takes a `Task<TResult>` and returns an `IObservable<TResult>`. The "async" in the name refers to the `async`/`await` model with which C# supports asynchronous blocking operations.
