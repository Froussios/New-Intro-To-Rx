...
There is also an [overload that converts a Task (non generic) to an IObservable<Unit>](https://msdn.microsoft.com/en-us/library/system.reactive.threading.tasks.taskobservableextensions). There you will also see some methods for creating Tasks out of IObservables. Often you won't need them as the *await* keyword works on IObservable as well:
